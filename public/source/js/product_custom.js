	...
	*/
	...
	function initQuantity()
	{
		...
		if($('.product_quantity').length)
		{
			...
			var product_price = $('#prodt_price').text();
			product_price = product_price.replace(/\./g,"");
			product_price = parseInt(product_price.replace(" đ",""));
			...
			incButton.on('click', function()
			{
				...
				if(originalVal < productLimit)
				{
					endVal = parseFloat(originalVal) + 1;
					input.val(endVal);
					var new_price = product_price*endVal;
					$('#prodt_price').html(addCommas(new_price)+' đ');
				}
			});
